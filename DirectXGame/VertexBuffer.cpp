...
void VertexBuffer::Create(const UINT size, const UINT stride) {
	...
	// 頂点バッファビューの設定
	vertexBufferView_.BufferLocation = vertexBuffer->GetGPUVirtualAddress(); // リソースの先頭のアドレスから
	vertexBufferView_.SizeInBytes = size;                                    // リソースのサイズ
	vertexBufferView_.StrideInBytes = stride;                                // 1つの頂点サイズ

	// 生成した頂点バッファをメンバ変数に格納
	vertexBuffer_ = vertexBuffer;
}

// 生成した VertexBufferを返す
ID3D12Resource* VertexBuffer::Get() { return vertexBuffer_; }

// 生成した VertexBufferViewを返す
